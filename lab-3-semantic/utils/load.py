import json
import codecs


from model.analysis import EPSILON_SIGN
from model.analysis import TERMINAL_SIGN


def expand_list(n_list):
    f_list = []

    for item in n_list:
        if isinstance(item, (list, tuple)):
            f_list.extend(expand_list(item))
        else:
            f_list.append(item)

    # 返回压平的嵌套列表.
    return f_list


class Grammar(object):
    """
    扩展 LL 文法, in_call 表示调用语义动作函数的标号.
    """

    def __init__(self, left, right, in_call=None):
        self._left = left
        self._right = right
        self._in_call = in_call

    @property
    def left(self):
        return self._left

    @property
    def right(self):
        return self._right

    @property
    def in_call(self):
        return self._in_call

    def __str__(self):
        if self._in_call is not None:
            return self._left + " -->> " + " ".join(self._right) + " (" + str(self._in_call) + ")"
        else:
            return self._left + " -->> " + " ".join(self._right) + " (None)"

    def __repr__(self):
        return self.__str__()

    def __len__(self):
        """
        专门用于计算下一步项目集用的, 在计算右
        向长度的时候去除掉空串, 并不计左步.
        """

        return len([s for s in self._right if s != EPSILON_SIGN])


class Token(object):
    """
    词法解析的一个结果, 包含词法器爬取值和类型两个部分.
    """

    def __init__(self, category, value):
        """
        所谓 category 类别就是能被语法分析识别出来, 而
        其 value 则是其表示字面意义, 对语义分析有用.
        """

        self._category = category
        self._value = value

    @property
    def value(self):
        return self._value

    @property
    def category(self):
        return self._category

    def __str__(self):
        return str(self._category) + "({})".format(self._value)

    def __repr__(self):
        return self.__str__()


class DataBuffer(object):
    """
    输入到词法分析器中做词法分析的字符缓冲.
    """

    def __init__(self, code_list):
        """
        这里输入是一个二维的字符表. 第一维表示行号, 第二
        维是改行包含的所有字符 (除了空格和空行以外).
        """

        self._code_list = code_list
        self._row_index = 0
        self._column_index = -1

    @property
    def char(self):
        """
        返回 None 表示已经到达文件的末尾了.
        """

        try:
            return self._code_list[self._row_index][self._column_index]
        except IndexError:
            return None

    @classmethod
    def from_file(cls, file_path):
        code_list = []

        with codecs.open(file_path, "r", encoding="utf-8") as fr:
            for line in fr.readlines():

                # 不要去掉空行, 这样至少保证每行一个字符.
                item_list = [list(w) for w in list(line)]
                code_list.append(expand_list(item_list))

        # 从文件构造对象并做返回.
        return cls(code_list)

    def move(self):
        self._column_index += 1

        # 如果列越界则读取下一行的元素.
        if self._column_index == len(self._code_list[self._row_index]):
            self._row_index += 1
            self._column_index = 0

    def back(self):
        self._column_index -= 1

        # 如果列越界则回退去读上一行.
        if self._column_index < 0:
            self._row_index -= 1
            self._column_index = len(self._code_list[self._row_index]) - 1

    def lexical_error(self, message):
        return message + " in " + "<line: {:3d}, index: {:3d}>".format(self._row_index, self._column_index)

    def __str__(self):
        return str(self._code_list)

    def __repr__(self):
        return self.__str__()


def lexicon_scan(code_path, ontology_path):
    """
    给定代码文本和预定义关键词字典进行词法分析.
    """

    buffer = DataBuffer.from_file(code_path)
    with codecs.open(ontology_path, "r", encoding="utf-8") as fr:
        ontology = json.load(fr)

    # 以二元组的形式存储爬取的结果.
    parse_list, error_list = [], []

    while buffer.char:
        buffer.move()

        # 如果没有输入了则退出.
        if buffer.char is None:
            break

        # 识别整数或者浮点数.
        if buffer.char.isdigit():
            int_value = 0
            while buffer.char.isdigit():
                int_value = int_value * 10 + int(buffer.char)
                buffer.move()

            if buffer.char != ".":
                buffer.back()
                parse_list.append(("INT", str(int_value)))
                continue

            float_value = str(int_value) + "."
            buffer.move()
            while buffer.char.isdigit():
                float_value += buffer.char
                buffer.move()
            buffer.back()

            parse_list.append(("FLOAT", str(float_value)))
            continue

        # 识别标识符或者关键字.
        if buffer.char.isalpha() or buffer.char == "_":
            string = ""

            while buffer.char.isalpha() or buffer.char.isdigit() or buffer.char == "_":
                string += buffer.char
                buffer.move()
            buffer.back()

            if string in ontology["keyword"]:
                parse_list.append((string, "-"))
            else:
                parse_list.append(("ID", string))
            continue

        # 识别出代码中的分隔符.
        if buffer.char in ontology["separator"]:
            parse_list.append((buffer.char, "SEP"))
            continue

        # 识别出代码中的操作数.
        if buffer.char in ontology["operator"]:
            operator = buffer.char

            buffer.move()
            next_char = buffer.char

            if next_char in ontology["operator"]:
                operator += next_char
            else:
                buffer.back()

            parse_list.append((operator, "OP"))
            continue

        # 如果以上情况都不满足则是错误的字符.
        if buffer.char.strip() != "":
            error_list.append(buffer.lexical_error("unknown character: {}".format(buffer.char)))

    # 返回词汇爬取的结果和恐慌模式的报错.
    lexicon_list = [Token(*p) for p in parse_list]
    return lexicon_list, error_list


def read_grammar(file_path):
    """
    读取定义语法类和预标注的终结符和非终结符集合.
    """

    with codecs.open(file_path, "r", encoding="utf-8") as fr:
        ontology_dict = json.load(fr)

    terminal_set = set(ontology_dict["terminal"])
    phrase_set = set(ontology_dict["non-terminal"])

    # 这里将 LL 文法 L-SDD 拓展为 R-SDD.
    medium_count, grammar_list = 0, []

    for product in ontology_dict["production"]:
        pair = product.strip().split("-->>")

        left = pair[0].strip()
        right = pair[1].strip().split()

        # 避免使替换新生成的文法覆盖原子文法.
        temp_list = []

        for idx in range(0, len(right) - 1):
            if right[idx].startswith("ONTOLOGY"):
                medium_call = right[idx]

                # 将嵌入语义动作置换为标记非终结符.
                medium_name = "MEDIUM_" + str(medium_count)
                medium_count += 1
                phrase_set.add(medium_name)

                temp_list.append(Grammar(
                    medium_name, [EPSILON_SIGN], medium_call
                ))
                right[idx] = medium_name

        if right[-1].startswith("ONTOLOGY"):
            grammar = Grammar(left, right[:-1], right[-1])
        else:
            grammar = Grammar(left, right)

        grammar_list.append(grammar)
        grammar_list.extend(temp_list)

    terminal_set.add(EPSILON_SIGN)
    terminal_set.add(TERMINAL_SIGN)
    return grammar_list, terminal_set, phrase_set
